...
import { ServiceMonsterMessage } from '../../../database/models/servicemonster.message.tdo';
...
export class RoomsService implements OnGatewayInit {
  ...
  async onJoin(
    ...
    @MessageBody() data: ServiceMonsterMessage,
  ) {
    try {
      ...
      const messages = await this.repoService.serviceMonsterRepository.find({
  ...
  async sendMessage(
    ...
    try {
      ...
      await this.amqpConnection.publish('serviceMonster-message', '', {
        content: data.content,
        user_name,
        email,
        live_slug,
